...
class FrameProcessor():    
    ...
    def process_frame(self):
        ...
        self.original_frame = cv2.resize(self.original_frame, (1920, 1080)) #
        original_shape = self.original_frame.shape  # 1920 1080 our initial image
        ...
        frame = cv2.resize(self.original_frame, (self.input_shape[1], self.input_shape[0]), 
                           interpolation = cv2.INTER_CUBIC)  # here we have our original image converted in 512*256
        toc_read = time.time()
        ...
        ###########  here is yolo algorithm to get segmentation
        frame_name = os.path.splitext(os.path.basename(self.input_frame.strip()))[0]
        road_mask = np.load(f'/home/k64684/ultralytics/numpy_results_last/{frame_name}.npy')
        segmented_frame = np.load(f'/home/k64684/ultralytics/numpy_results_last/{frame_name}_segmented_frame.npy')
        ...
        # pcd_raw.points = o3d.utility.Vector3dVector(points3D)
        # pcd_raw.colors = o3d.utility.Vector3dVector(colors)

        # o3d.io.write_point_cloud("/home/k64684/semantic-depth/road3D_raw.ply", pcd_raw)
        # print("Saved raw road point cloud before processing")
        ###################################

        print("Dtype of road3D:", road3D.dtype)
        print("Dtype of road_colors:", road_colors.dtype)
        #: FENCE
        ...
        road3D, road_colors = pcl.remove_from_to(road3D, road_colors, 2, 0.0, 7.0)   # it was 7
        ...
        road3D_pcd = o3d.geometry.PointCloud()
        #road3D_pcd.points = Vector3dVector(road3D)
        #road3D_pcd.colors = Vector3dVector(road_colors)
        road3D_pcd.points = o3d.utility.Vector3dVector(road3D)
        road3D_pcd.colors = o3d.utility.Vector3dVector(road_colors)
        ...
        #####################################
        cl, ind = road3D_pcd.remove_statistical_outlier(nb_neighbors=10, std_ratio=0.5)
        inlier_cloud = road3D_pcd.select_by_index(ind)

        # Radius outlier removal
        cl, ind = inlier_cloud.remove_radius_outlier(nb_points=80, radius=0.5)
...
#####################################################
        ...
        pcd_processed.points = o3d.utility.Vector3dVector(road3D)
        pcd_processed.colors = o3d.utility.Vector3dVector(road_colors)

        o3d.io.write_point_cloud("/home/k64684/semantic-depth/road3D_processed.ply", pcd_processed)
        print("Saved processed road point cloud after cleaning")
        # 
        left_pt_rw, right_pt_rw = pcl.get_end_points_of_road(road3D, 
                                                                   self.depth-0.02)
        print(left_pt_rw, right_pt_rw)
...
class DepthFrame():
    def __init__(self, is_city=False, encoder='vgg', input_height=256, input_width=512, 
        ...
        if self.is_city:
            ...
            self.cx = 256.72544465
            self.cy = 124.839484
            self.b  = 1
            if self.f is None:
                self.f = 476.16994442 # works well for most images in the Munich test set
    ...
    def compute_3D_points(self, disp):
        ...
        alpha = 0.0
        newCam = np.array([
        [476.16994442,   0.0, 256.72544465],
        [0.0, 369.36448577,   124.839484],
        [0.0, 0.0, 1.0]
        ], dtype=np.float64)
        dist = np.array([-2.48182098e-01,  2.86245340e-01,  7.67177547e-05,  1.07812253e-03, -3.41622955e-01], dtype=np.float64)
        H_disp, W_disp = disp.shape[:2]
        newCam_opt, roi = cv2.getOptimalNewCameraMatrix(newCam, dist, (W_disp, H_disp), alpha, (W_disp, H_disp))
        map1, map2 = cv2.initUndistortRectifyMap(newCam, dist, R=np.eye(3), newCameraMatrix=newCam_opt,
                                         size=(W_disp, H_disp), m1type=cv2.CV_32FC1)


        disp_undist = cv2.remap(disp.astype(np.float32), map1, map2, interpolation=cv2.INTER_LINEAR)
        f = newCam_opt[0,0]
        cx = newCam_opt[0,2]
        cy = newCam_opt[1,2]
...
def main():
    ...
    parser.add_argument("--input_folder", help="Path to all frames we want to work with.", 
                        #default="data/test_images_munich/normal_images"
                        default="/home/k64684/data_from_Marcus/Export_Ulm")
    ...
    parser.add_argument("--semantic_model", help="Path to semantic segmentation model.", 
                       default="models/sem_seg/30-Epochs-cityscapes") # 100-Epochs-roborace750
    ...
    # Input size
    input_shape = (args.input_height, args.input_width)  # 256 512
    ...
    # A series of frames
        ...
        # input_frames = {"1.jpg": 6, "2.jpg": 6, "3.jpg": 6, "4.jpg": 6, "5.jpg": 6, "6.jpg": 6, "7.jpg": 6, "8.jpg": 6, "9.jpg": 6, "10.jpg": 6, "11.jpg": 6, "12.jpg": 6, "13.jpg": 6, "14.jpg": 6, "15.jpg": 6}
        #input_frames = {"1.jpg" : 6}
        
        
        input_frames = {"1.jpg": 6, "2.jpg": 6, "3.jpg": 6, "4.jpg": 6, "5.jpg": 6, "6.jpg": 6, "7.jpg": 6, "8.jpg": 6, "9.jpg": 6, "10.jpg": 6, "11.jpg": 6, "12.jpg": 6, "13.jpg": 6, "14.jpg": 6, "15.jpg": 6,"16.jpg": 6,"17.jpg": 6,"18.jpg": 6,"19.jpg": 6,"20.jpg": 6,"21.jpg": 6,"22.jpg": 6,"23.jpg": 6,"24.jpg": 6,"25.jpg": 6,"26.jpg": 6,"27.jpg": 6,"28.jpg": 6,"29.jpg": 6}
        input_frames = {
            "1.png": 0.0,
            "2.png": 0.0,
            "3.png": 0.0,
        ...
        sum_error = 0.0
        depth_m = 8
        max_error = -np.inf   # start lower than any possible error
        min_error = np.inf    # start higher than any possible error
        error_list = []       # store all per-frame signed errors
        abs_error_list = []   # store per-frame absolute errors
        output_csv_path = "/home/k64684/semantic-depth/road_width_results.csv"
        csv_file = open(output_csv_path, mode="w", newline="")
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(["frame", "depth_m", "road_width_m", "error (real width - estimated width)", "sum_error"])
        # csv
        ...
        if args.f is None:
            ...
            #focal_lengths = [239.6]
            #focal_lengths = [422.767215095]
            focal_lengths = [476.16994442]    # normal fx!!!!!
            #focal_lengths = [468.79080982]
            #focal_lengths = [419.38081341404113036040342452419]
            ...
            for f in focal_lengths:
                ...
                for input_frame, real_distance in sorted(input_frames.items()):
                    ...
                    ######################
                    num_frames_measured += 1   # <-- increment counter
                    error = real_width - dist_rw
                    abs_error = abs(error)
                    sum_error += abs(error)
                    ...
                    if abs_error < min_error:
                        min_error = abs_error
                    frame_idx = os.path.splitext(os.path.basename(input_frame))[0]
                    frame_idx = int(frame_idx) 
                    csv_writer.writerow([frame_idx, depth_m, f"{dist_rw:.2f}", error, sum_error])   # error (real width - estimated width)
                ...
                ###############################################
                mae = sum_error / num_frames_measured
                std_error = np.std(error_list)  # how consistent your measerements are
                print("The sum error", sum_error)
                print(f"Mean absolute error (MAE): {mae:.3f} m")
                print(f"Standard deviation: {std_error:.3f} m")  # 0.07 → very consistent; 0.5 → fluctuates a lot
                print(f"Maximum per-frame error: {max_error:.3f} m")
                print(f"Minimum per-frame error: {min_error:.3f} m")
                print("Done. CSV, video, and PLY files saved.")
                ##############################################
